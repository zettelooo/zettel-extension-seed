...
> = function ({ api }, onUpdate?) {
  let pageExtensionData = this.data.page.extensionManagedData[api.extensionHeader.id] as PageExtensionData

  this.register(
    this.watch(
      data => data.page.extensionManagedData[api.extensionHeader.id] as PageExtensionData,
      (newValue, oldValue) => {
